...
function getIpInfo() {
    ...
    if ("true" === clock_default_rectangle_enable) {
        fetch(`https://restapi.amap.com/v3/geocode/regeo?key=${gaud_map_key}&location=${clock_rectangle}`)
            ...
            .then(data => {
                if ("1" === data.status) {
                    ...
                    e.qweather_url = `https://devapi.qweather.com/v7/weather/now?location=${clock_rectangle}&key=${qweather_key}`;
                    fetchWeatherData(e);
                } else {
                    console.error('高德地图位置信息获取失败:', data.message);
                    fetchTencentLocation(e);
                }
            })
            .catch(error => {
                console.error('高德地图请求失败:', error);
                fetchTencentLocation(e);
            });
    } else {
        fetchTencentLocation(e);
...
function fetchTencentLocation(e) {
    $.ajax({
        type: 'get',
        ...
        dataType: 'jsonp',
        jsonpCallback: 'handleLocationData',
        timeout: 10000,
        ...
        success: function(res) {
            if (res.status === 0) {
                ...
                e.city = ad_info.city; // 使用腾讯地图API获取的城市信息
                e.qweather_url = `https://devapi.qweather.com/v7/weather/now?location=${location.lng},${location.lat}&key=${qweather_key}`;
                fetchWeatherData(e);
            } else {
                console.error('腾讯地图位置信息获取失败:', res.message);
                fetchWeatherData(e); // 使用默认城市
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.error('腾讯地图请求失败:', textStatus, errorThrown);
            fetchWeatherData(e); // 使用默认城市
        }
    });
}
    ...
    fetch(e.qweather_url)
        ...
        .then(weatherData => {
            if (document.getElementById("hexo_electric_clock")) {
                clockUpdateTime(weatherData, e.city);
            }
        })
        .catch(error => {
